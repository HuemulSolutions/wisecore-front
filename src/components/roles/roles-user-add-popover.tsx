...
export interface RolesUserAddPopoverProps {
  /** Usuarios ya asignados o ya en staging — se excluyen de los resultados. */
  excludeUserIds: Set<string>
  onUserAdded: (user: User, opts?: { created?: boolean }) => void
  disabled?: boolean
}
...
/**
 * Popover "Agregar usuario" del panel de detalle de un rol — espejo de
 * `UsersRoleAddPopover` para la dirección inversa: solo busca/lista usuarios
 * existentes. Crear un usuario nuevo se hace desde el botón "Crear usuario"
 * de `RolesDetailUsersTab`, que abre `CreateUserSheet` completo — ver
 * ia context/inline-create-entity-in-sheet-guide.md.
 */
export function RolesUserAddPopover({
  excludeUserIds,
  onUserAdded,
  disabled,
}: RolesUserAddPopoverProps) {
  const { t } = useTranslation(["roles", "users", "common"])
  ...
  const { data, isLoading, isFetching } = useUsers(
    open && !!selectedOrganizationId && !!organizationToken,
