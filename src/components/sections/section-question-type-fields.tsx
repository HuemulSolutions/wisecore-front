...
export function SectionQuestionTypeFields({
      ...
      <div className="space-y-2">
        {options.map((opt, i) => (
          <div key={opt.id || i} className="flex items-center gap-2">
            <span className="w-4 shrink-0 text-xs" style={{ color: "#94a3b8" }}>{i + 1}</span>
            <Input
              value={opt.label}
              onChange={(e) => {
            ...
            >
              {t("form.formFields.removeOption")}
            </button>
          </div>
        ))}
        <button
  ...
  switch (qt) {
    // ── Texto / email / fecha / hora: sin ajustes propios ───────────────────
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            <HuemulField
              type="number"
              value={min}
              onChange={(v) => onUpdate({ min_value: Math.min(1, Math.max(0, Number(v) || 0)) })}
              min={0}
              max={1}
              disabled={isPending}
              className="w-18.5"
            ...
            <HuemulField
              type="number"
              value={max}
              onChange={(v) => onUpdate({ max_value: Math.min(10, Math.max(2, Number(v) || 2)) })}
              min={2}
              max={10}
              disabled={isPending}
              className="w-18.5"
    ...
    // ── Etiqueta: separador visual, sin ajustes propios (la vista previa de la
    // derecha ya muestra el separador) ──────────────────────────────────────
    ...
    // ── Fallback: slug no contemplado, sin ajustes propios ──────────────────
    default:
      return null;
  }
}
