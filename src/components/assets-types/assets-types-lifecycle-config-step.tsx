...
export function ConfigStepContent({
  documentTypeId,
  stepType,
  stepLabel,
  hasSla,
  ...
  const [accessType, setAccessType] = useState<"all" | "owner" | "custom">("owner")
  ...
  const handleEdit = () => {
    setSnapshot({ accessType, roleIds, slaEnabled, slaValue, slaUnit, validFrom, validTo })
    setIsEditing(true)
  ...
  // Sync local state when step data loads
  useEffect(() => {
    if (step) {
      setAccessType(
        step.access_type === "all"
          ? "all"
          : step.access_type === "custom"
          ? "custom"
          : "owner"
      )
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col gap-3 py-2">
        ...
        {hasValidity && <Skeleton className="h-10 w-full rounded-md" />}
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {t(`lifecycle.stepDescriptions.${stepType}`, {
              defaultValue: t("lifecycle.configStepDescription"),
            })}
          </p>
        </div>
      ...
      {/* Validity date range (only for steps that support it) */}
      {hasValidity && (
        ...
        <HuemulField
          ...
          onChange={(v) => {
            setAccessType(v as "all" | "owner" | "custom")
            if (v !== "custom") setRoleIds([])
          }}
          disabled={!isEditing}
        />
      </div>
