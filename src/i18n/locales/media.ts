const translations = {
  title: { en: "Resources", es: "Recursos" },
  totalItems: { en: "Total", es: "Total" },
  refreshSuccess: { en: "Resource list refreshed", es: "Lista de recursos actualizada" },
  refreshError: { en: "Failed to refresh", es: "Error al actualizar" },
  loadError: { en: "Failed to load resources", es: "Error al cargar los recursos" },
  noOrganization: {
    en: "Select an organization to view resources",
    es: "Seleccionar una organización para ver los recursos",
  },
  emptyTitle: { en: "No resources found", es: "No se encontraron recursos" },
  emptyDescription: {
    en: "No resources match the current filters.",
    es: "No hay recursos que coincidan con los filtros actuales.",
  },
  emptySelectParentTitle: {
    en: "Select an entity",
    es: "Seleccionar una entidad",
  },
  emptySelectParentDescription: {
    en: "Choose a parent entity to view its resources.",
    es: "Elegir una entidad para ver sus recursos.",
  },
  filters: {
    title: { en: "Filters", es: "Filtros" },
    level: { en: "Level", es: "Nivel" },
    mediaType: { en: "File Type", es: "Tipo de Archivo" },
    allMediaTypes: { en: "All types", es: "Todos los tipos" },
    mediaTypes: {
      pdf: { en: "PDF", es: "PDF" },
      docx: { en: "Word (DOCX)", es: "Word (DOCX)" },
      xlsx: { en: "Excel (XLSX)", es: "Excel (XLSX)" },
      png: { en: "PNG image", es: "Imagen PNG" },
      jpg: { en: "JPG image", es: "Imagen JPG" },
      pptx: { en: "PowerPoint (PPTX)", es: "PowerPoint (PPTX)" },
      txt: { en: "Text (TXT)", es: "Texto (TXT)" },
    },
    parentPlaceholder: { en: "Select…", es: "Seleccionar…" },
    clear: { en: "Clear filters", es: "Limpiar filtros" },
    levels: {
      organization: { en: "Organization", es: "Organización" },
      document_type: { en: "Asset Type", es: "Tipo de Activo" },
      document: { en: "Asset", es: "Activo" },
      execution: { en: "Execution", es: "Ejecución" },
      template: { en: "Template", es: "Plantilla" },
    },
  },
  picker: {
    title: { en: "Select", es: "Seleccionar" },
    searchPlaceholder: { en: "Search assets…", es: "Buscar assets…" },
    clearSearch: { en: "Clear search", es: "Limpiar búsqueda" },
    noResults: { en: "No results found", es: "No se encontraron resultados" },
    parentEmpty: { en: "Select…", es: "Seleccionar…" },
    currentAsset: { en: "Current asset", es: "Asset actual" },
    currentVersion: { en: "Current version", es: "Versión actual" },
    modeAsset: { en: "Asset", es: "Asset" },
    modeVersion: { en: "Version", es: "Versión" },
  },
  templateTab: {
    title: { en: "Resources", es: "Recursos" },
    description: {
      en: "Manage resources attached to this template",
      es: "Gestionar los recursos adjuntos a esta plantilla",
    },
    empty: {
      en: "No resources attached to this template",
      es: "Sin recursos adjuntos a esta plantilla",
    },
    emptyDescription: {
      en: "Upload a file to attach it to this template.",
      es: "Subir un archivo para adjuntarlo a esta plantilla.",
    },
  },
  listSheet: {
    title: { en: "Attached resources", es: "Recursos adjuntos" },
    titleWithName: { en: "Resources — {{name}}", es: "Recursos — {{name}}" },
    description: {
      en: "Resources uploaded to this document or version.",
      es: "Recursos subidos a este documento o versión.",
    },
    emptyTitle: { en: "No resources attached", es: "Sin recursos adjuntos" },
    emptyDescription: {
      en: "No resources have been uploaded here yet.",
      es: "Todavía no se subieron recursos aquí.",
    },
    scope: {
      trigger: { en: "Change version", es: "Cambiar versión" },
      document: { en: "Full document", es: "Documento completo" },
      versionsHeading: { en: "Versions", es: "Versiones" },
    },
  },
  detail: {
    details: { en: "Details", es: "Detalles" },
    name: { en: "Name", es: "Nombre" },
    origin: { en: "Origin", es: "Origen" },
    createdAt: { en: "Created", es: "Creado" },
    description: { en: "Description", es: "Descripción" },
    descriptionPlaceholder: { en: "Brief description of the file", es: "Breve descripción del archivo" },
    versions: { en: "Versions", es: "Versiones" },
    current: { en: "current", es: "actual" },
    currentVersionBadge: { en: "v{{version}} · current", es: "v{{version}} · actual" },
    download: { en: "Download", es: "Descargar" },
    expand: { en: "View fullscreen", es: "Ver en pantalla completa" },
    noVersions: { en: "No versions found", es: "No se encontraron versiones" },
    uploadVersion: { en: "Upload version", es: "Subir versión" },
    uploadShort: { en: "Upload", es: "Subir" },
    uploadingVersion: { en: "Uploading version…", es: "Subiendo versión…" },
    uploadVersionSuccess: { en: "New version uploaded successfully", es: "Nueva versión subida correctamente" },
    uploadVersionError: { en: "Failed to upload version", es: "Error al subir la versión" },
    dropTitle: { en: "Drop to create v{{version}}", es: "Suelta para crear la v{{version}}" },
    dropHint: {
      en: "The current version moves to history, nothing is lost.",
      es: "La versión actual pasa al historial, no se pierde.",
    },
    deleteVersion: { en: "Delete version", es: "Eliminar versión" },
    deleteVersionTitle: { en: "Delete version?", es: "¿Eliminar versión?" },
    deleteVersionDescription: { en: "Version v{{version}} will be permanently deleted. This action cannot be undone.", es: "La versión v{{version}} será eliminada permanentemente. Esta acción no se puede deshacer." },
    deleteVersionConfirm: { en: "Delete", es: "Eliminar" },
    deleteVersionSuccess: { en: "Version deleted successfully", es: "Versión eliminada correctamente" },
    deleteMedia: { en: "Delete", es: "Eliminar" },
    deleteMediaTitle: { en: "Delete resource?", es: "¿Eliminar recurso?" },
    deleteMediaDescription: { en: "This resource and all its versions will be permanently deleted. This action cannot be undone.", es: "Este recurso y todas sus versiones serán eliminados permanentemente. Esta acción no se puede deshacer." },
    deleteMediaConfirm: { en: "Delete", es: "Eliminar" },
    deleteMediaSuccess: { en: "Resource deleted successfully", es: "Recurso eliminado correctamente" },
    edit: { en: "Edit", es: "Editar" },
    editSuccess: { en: "Resource updated successfully", es: "Recurso actualizado correctamente" },
    editError: { en: "Failed to update the resource", es: "Error al actualizar el recurso" },
  },
  upload: {
    title: { en: "Upload Resource", es: "Subir Recurso" },
    submit: { en: "Upload", es: "Subir" },
    dropzone: { en: "Drag & drop a file, or click to select", es: "Arrastrá un archivo o hacé clic para seleccionar" },
    dropzoneHint: { en: "Allowed: PDF, DOCX, XLSX, PNG, JPG, CSV, PPTX, TXT", es: "Permitidos: PDF, DOCX, XLSX, PNG, JPG, CSV, PPTX, TXT" },
    changeFile: { en: "Change file", es: "Cambiar archivo" },
    name: { en: "Name (optional)", es: "Nombre (opcional)" },
    namePlaceholder: { en: "Leave empty to use filename", es: "Dejar vacío para usar el nombre del archivo" },
    summary: { en: "Summary (optional)", es: "Resumen (opcional)" },
    summaryPlaceholder: { en: "Brief description of the file", es: "Breve descripción del archivo" },
    success: { en: "File uploaded successfully", es: "Archivo subido correctamente" },
    invalidFileType: { en: "Invalid file type. Allowed: {{formats}}", es: "Tipo de archivo no válido. Permitidos: {{formats}}" },
  },
  generate: {
    button: { en: "Generate with AI", es: "Generar con IA" },
    title: { en: "Generate image with AI", es: "Generar imagen con IA" },
    prompt: { en: "Prompt", es: "Prompt" },
    promptPlaceholder: {
      en: "e.g. Minimalist illustration of a data pipeline, blue tones",
      es: "ej. Ilustración minimalista de un pipeline de datos, en tonos azules",
    },
    promptHelp: {
      en: "The prompt is stored as the resource name and summary.",
      es: "El prompt se guarda como nombre y resumen del recurso.",
    },
    name: { en: "Name", es: "Nombre" },
    namePlaceholder: { en: "e.g. Kitten picture", es: "ej. Imagen gatito" },
    nameHelp: {
      en: "Optional. If left empty, the prompt is used as the name.",
      es: "Opcional. Si se deja vacío, se usa el prompt como nombre.",
    },
    model: { en: "Model", es: "Modelo" },
    modelAuto: { en: "Automatic", es: "Automático" },
    modelHelp: {
      en: "Choose which configured model generates the image.",
      es: "Elegir qué modelo configurado genera la imagen.",
    },
    modelEmpty: {
      en: "No models with image generation are configured.",
      es: "No hay modelos con generación de imágenes configurados.",
    },
    saveAsVersion: {
      en: "Save as a new version of \"{{name}}\"",
      es: "Guardar como nueva versión de \"{{name}}\"",
    },
    saveAsVersionHelp: {
      en: "If disabled, the result is saved as a new, separate image.",
      es: "Si se desactiva, el resultado se guarda como una imagen nueva y separada.",
    },
    sessionImage: { en: "this image", es: "esta imagen" },
    regenerateWithAI: { en: "Regenerate with AI", es: "Regenerar con IA" },
    aspectRatio: { en: "Aspect ratio", es: "Relación de aspecto" },
    format: { en: "Format", es: "Formato" },
    ratios: {
      square: { en: "Square (1:1)", es: "Cuadrada (1:1)" },
      landscape: { en: "Landscape (16:9)", es: "Horizontal (16:9)" },
      portrait: { en: "Portrait (9:16)", es: "Vertical (9:16)" },
      standard: { en: "Standard (4:3)", es: "Estándar (4:3)" },
      tall: { en: "Tall (3:4)", es: "Vertical clásica (3:4)" },
    },
    submit: { en: "Generate image", es: "Generar imagen" },
    regenerate: { en: "Generate again", es: "Generar de nuevo" },
    generating: { en: "Generating image…", es: "Generando imagen…" },
    generatingHint: {
      en: "This can take up to a minute. Keep this panel open.",
      es: "Puede tardar hasta un minuto. Mantener este panel abierto.",
    },
    elapsed: { en: "{{seconds}}s elapsed", es: "{{seconds}}s transcurridos" },
    emptyPreview: { en: "Your image will appear here", es: "La imagen aparecerá aquí" },
    emptyHint: {
      en: "You'll be able to download it or ask for another version without leaving the panel.",
      es: "Podrás descargarla o pedir otra versión sin salir del panel.",
    },
    previewAlt: { en: "Generated image", es: "Imagen generada" },
    expand: { en: "View fullscreen", es: "Ver en pantalla completa" },
    previewExpired: { en: "The preview link expired", es: "El enlace de vista previa expiró" },
    reloadPreview: { en: "Reload preview", es: "Recargar vista previa" },
    insert: { en: "Insert in asset", es: "Insertar en el activo" },
    download: { en: "Download", es: "Descargar" },
    discard: { en: "Discard", es: "Descartar" },
    history: { en: "Previous generations", es: "Generaciones anteriores" },
    session: { en: "This session", es: "Esta sesión" },
    sessionCount_one: { en: "{{count}} generation", es: "{{count}} generación" },
    sessionCount_other: { en: "{{count}} generations", es: "{{count}} generaciones" },
    savedNote: {
      en: "Every generation is saved to Resources at organization level.",
      es: "Cada generación se guarda en Recursos a nivel organización.",
    },
    success: { en: "Image generated and saved", es: "Imagen generada y guardada" },
    versionSuccess: {
      en: "{{name}} - v{{version}} generated and saved",
      es: "{{name}} - v{{version}} generada y guardada",
    },
    error: { en: "Failed to generate the image", es: "Error al generar la imagen" },
    discardSuccess: { en: "Image discarded", es: "Imagen descartada" },
    discardVersionSuccess: { en: "Version discarded", es: "Versión descartada" },
    discardError: { en: "Failed to discard the image", es: "Error al descartar la imagen" },
    hiddenByFilters: {
      en: "The new image was saved at organization level; clear the filters to see it.",
      es: "La nueva imagen se guardó a nivel organización; limpia los filtros para verla.",
    },
    errors: {
      invalidModel: {
        en: "The selected model does not support image generation.",
        es: "El modelo seleccionado no admite generación de imágenes.",
      },
      notImage: {
        en: "The selected resource is not an image and can't be versioned this way.",
        es: "El recurso seleccionado no es una imagen y no se puede versionar así.",
      },
      versionForbidden: {
        en: "You don't have permission to add a new version to this resource.",
        es: "No hay permiso para agregar una nueva versión a este recurso.",
      },
    },
  },
}

export default translations
