const translations = {
  refresh: { en: "Refresh", es: "Actualizar" },
  save: { en: "Save", es: "Guardar" },
  saving: { en: "Saving...", es: "Guardando..." },
  cancel: { en: "Cancel", es: "Cancelar" },
  confirm: { en: "Confirm", es: "Confirmar" },
  discard: { en: "Discard", es: "Descartar" },
  delete: { en: "Delete", es: "Eliminar" },
  deleting: { en: "Deleting...", es: "Eliminando..." },
  create: { en: "Create", es: "Crear" },
  creating: { en: "Creating...", es: "Creando..." },
  update: { en: "Update", es: "Actualizar" },
  updating: { en: "Updating...", es: "Actualizando..." },
  configure: { en: "Configure", es: "Configurar" },
  configuring: { en: "Configuring...", es: "Configurando..." },
  tryAgain: { en: "Try Again", es: "Reintentar" },
  close: { en: "Close", es: "Cerrar" },
  edit: { en: "Edit", es: "Editar" },
  done: { en: "Done", es: "Listo" },
  add: { en: "Add", es: "Agregar" },
  execute: { en: "Execute", es: "Ejecutar" },
  executing: { en: "Executing...", es: "Ejecutando..." },
  export: { en: "Export", es: "Exportar" },
  exporting: { en: "Exporting...", es: "Exportando..." },
  selectAll: { en: "Select all", es: "Seleccionar todos" },
  deselectAll: { en: "Deselect all", es: "Deseleccionar todos" },
  select: { en: "Select", es: "Seleccionar" },
  previous: { en: "Previous", es: "Anterior" },
  next: { en: "Next", es: "Siguiente" },
  actions: { en: "Actions", es: "Acciones" },
  expandRow: { en: "Expand row", es: "Expandir fila" },
  collapseRow: { en: "Collapse row", es: "Contraer fila" },
  name: { en: "Name", es: "Nombre" },
  created: { en: "Created", es: "Creado" },
  updated: { en: "Updated", es: "Actualizado" },
  email: { en: "Email", es: "Correo Electrónico" },
  status: { en: "Status", es: "Estado" },
  yes: { en: "Yes", es: "Sí" },
  no: { en: "No", es: "No" },
  search: { en: "Search", es: "Buscar" },
  viewGrid: { en: "Grid view", es: "Vista cuadrícula" },
  viewList: { en: "List view", es: "Vista lista" },
  active: { en: "Active", es: "Activo" },
  inactive: { en: "Inactive", es: "Inactivo" },
  pending: { en: "Pending", es: "Pendiente" },
  executionStatus: {
    pending: { en: "Pending", es: "Pendiente" },
    queued: { en: "Queued", es: "En cola" },
    importing: { en: "Importing", es: "Importando" },
    running: { en: "Running", es: "En ejecución" },
    generating: { en: "Generating", es: "Generando" },
    approving: { en: "Approving", es: "Aprobando" },
    paused: { en: "Paused", es: "Pausado" },
    completed: { en: "Completed", es: "Completado" },
    done: { en: "Completed", es: "Completado" },
    approved: { en: "Approved", es: "Aprobado" },
    failed: { en: "Failed", es: "Fallido" },
    import_failed: { en: "Import failed", es: "Importación fallida" },
    cancelled: { en: "Cancelled", es: "Cancelado" },
  },
  configured: { en: "Configured", es: "Configurado" },
  notConfigured: { en: "Not configured", es: "No configurado" },
  configureProvider: { en: "Configure Provider", es: "Configurar Proveedor" },
  updateProvider: { en: "Update Provider", es: "Actualizar Proveedor" },
  dataRefreshed: { en: "Data refreshed", es: "Datos actualizados" },
  refreshFailed: { en: "Failed to refresh data", es: "Error al actualizar los datos" },
  accessDenied: { en: "Access Denied", es: "Acceso Denegado" },
  noPermission: { en: "You don't have permission to access this page.", es: "No hay permiso para acceder a esta página." },
  goBack: { en: "Go Back", es: "Volver" },
  goToHome: { en: "Go to Home", es: "Ir a Inicio" },
  pickDate: { en: "Pick a date", es: "Seleccionar una fecha" },
  pickTime: { en: "Pick a time", es: "Seleccionar una hora" },
  pickDateTime: { en: "Pick a date & time", es: "Seleccionar fecha y hora" },
  selectDate: { en: "Select date", es: "Seleccionar fecha" },
  selectTime: { en: "Select time", es: "Seleccionar hora" },
  dateFrom: { en: "From", es: "Desde" },
  dateTo: { en: "To", es: "Hasta" },
  anyDate: { en: "Any date", es: "Cualquier fecha" },
  dateSingle: { en: "Single date", es: "Fecha exacta" },
  dateRange: { en: "Date range", es: "Rango de fechas" },
  selectPlaceholder: { en: "Select...", es: "Seleccionar..." },
  searchPlaceholder: { en: "Search...", es: "Buscar..." },
  chooseFile: { en: "Choose a file...", es: "Elegir un archivo..." },
  filesSelected: { en: "{{count}} files selected", es: "{{count}} archivos seleccionados" },
  noResults: { en: "No results found.", es: "Sin resultados." },
  loadMore: { en: "Load more", es: "Cargar más" },
  loading: { en: "Loading...", es: "Cargando..." },
  loadingMore: { en: "Loading more...", es: "Cargando más..." },
  noMoreResults: { en: "No more results", es: "No hay más resultados" },
  retry: { en: "Try again", es: "Reintentar" },
  and: { en: "and", es: "y" },
  undo: { en: "Undo", es: "Deshacer" },
  newFolder: { en: "New folder", es: "Nueva carpeta" },
  folderNamePlaceholder: { en: "Folder name", es: "Nombre de la carpeta" },
  createFolderHint: { en: "Enter to create · Esc to cancel", es: "Enter para crear · Esc para cancelar" },
  createFolderHintOrdered: { en: "Enter to create · Esc to cancel · it will sort with the folders", es: "Enter para crear · Esc para cancelar · se ordenará con las carpetas" },
  folderCreateError: { en: "The folder could not be created", es: "No se pudo crear la carpeta" },
  folderRenameError: { en: "The folder could not be renamed", es: "No se pudo renombrar la carpeta" },
  renameFolder: { en: "Rename", es: "Renombrar" },
  deleteFolder: { en: "Delete", es: "Eliminar" },
  moveToFolder: { en: "Move to folder", es: "Mover a carpeta" },
  removeFromFolder: { en: "Remove from folder", es: "Quitar de la carpeta" },
  itemsCount: { en: "{{count}} items", es: "{{count}} elementos" },
  emptyFolderHint: { en: "Empty folder — drag items here", es: "Carpeta vacía — arrastra elementos aquí" },
  pagination: {
    itemsPerPage: { en: "Items per page:", es: "Elementos por página:" },
    page: { en: "Page", es: "Página" },
    of: { en: "of", es: "de" },
    items: { en: "items", es: "elementos" },
    noResults: { en: "No results", es: "Sin resultados" },
    showing: { en: "Showing", es: "Mostrando" },
    perPage: { en: "Per page", es: "Por página" },
  },
  relativeTime: {
    justNow: { en: "Just now", es: "Ahora mismo" },
    minutesAgo: { en: "{{count}}m ago", es: "hace {{count}}m" },
    hoursAgo: { en: "{{count}}h ago", es: "hace {{count}}h" },
    yesterday: { en: "Yesterday", es: "Ayer" },
  },
}

export default translations
