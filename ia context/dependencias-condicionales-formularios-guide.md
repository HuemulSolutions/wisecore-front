...
- **Gap conocido**: `useLifecycleProgress` (panel "N de M" de las sheets de confirmación) calcula el progreso en el cliente listando TODOS los steps configurados del tipo de activo, sin saber cuáles aplican a la ejecución puntual. Con un step inaplicable por `depends_on`, el contador puede mostrar de más y `nextStep` puede anunciar un destino incorrecto. No hay mitigación cliente — requiere que `lifecycle_status` exponga los steps aplicables de la ejecución (pedido pendiente a backend). El efecto real (no exigir la etapa, no bloquear el avance) ya lo aplica el backend vía `GET /documents/{id}/content` → `lifecycle_status`; el gap es solo visual, dentro de las sheets de transición.
